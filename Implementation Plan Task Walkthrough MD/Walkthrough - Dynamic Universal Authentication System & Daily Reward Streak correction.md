...
### 1. Supabase Gateway Hook Token Authorization Bypass
- **Problem**: GoTrue returned an `unexpected_failure` / `Hook requires authorization token` 500 status code when trying to sign up.
- **Fix**: Created [config.toml](file:///f:/.gemini/antigravity/scratch/aqua-sort-flutter/supabase/config.toml) with `verify_jwt = false` for the `send-security-email` hook function and deployed it to project `zpwwjdiwcucwfuzyuiqu`. This prevents GoTrue's email hooks from being blocked by gateway JWT checks.

### 2. Welcome Screen Button Swap ([splash_screen.dart](file:///f:/.gemini/antigravity/scratch/aqua-sort-flutter/lib/features/auth/screens/splash_screen.dart))
...
### 3. Guest Status Mapping & Profile Log Out Mismatch
...
### 4. Purity Exchange Layout Overflows ([exchange_overlay.dart](file:///f:/.gemini/antigravity/scratch/aqua-sort-flutter/lib/features/lobby/widgets/exchange_overlay.dart))
