...
Integrate the official Flutter `in_app_purchase` package to replace the existing mock checkout dialogs. This will allow players to buy real premium upgrades (one-time purchase) and coin packs (consumable) using Google Play Billing.

---

## 💸 Where Does the Paid Money Go? (Payout Process)

When a player makes a purchase in the app, the transaction flows as follows:

```mermaid
flowchart LR
    A[Player Pays in App] -->|UPI / Cards / NetBanking| B(Google Play Billing)
    B -->|Deducts 15% Store Fee| C(Google Merchant Account)
    C -->|Direct Deposit| D[Your Bank Account]
```

1. **Transaction Processing**: The player pays Google Play using their linked local payment method (e.g., UPI, credit cards, carrier billing, NetBanking).
2. **Google Play Cut**: Google takes a **15% service fee** (for your first $1M USD in annual revenue). The remaining **85%** belongs to WebSpider Studios.
3. **Payout Deposit**: Google deposits the earnings directly into the bank account you linked in your **Merchant Profile** (configured in Step 1 of the Setup Guide).
4. **Payout Timing**: Payouts are transferred automatically on a monthly schedule (typically around the 15th of every month) for the prior month's earnings, once you hit the minimum payout threshold (usually $100 USD or ₹1000 INR).
...
| Product Name | Product ID | US/EU Price | India Price | Included Benefits & Coins |
| :--- | :--- | :--- | :--- | :--- |
| **Go Premium Upgrade** | `webspider_premium_upgrade` | $1.99 | ₹99 | **One-time purchase**:<br>• Permanent Ad-Free status<br>• Unlimited Pauses & Undos<br>• Exclusive Cyber Themes<br>• **Bonus: 500 Regular, 50 Silver, 20 Gold Coins** |
