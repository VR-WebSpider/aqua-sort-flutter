...
## ⚡ Fast, Non-Blocking, & Stuck-Free Checkout UX

To prevent the app from freezing, lagging, or getting stuck during purchases (which leads to cancellations and refunds):

1. **Asynchronous Non-Blocking Processing**: The billing lifecycle runs entirely in the background. The main UI thread remains completely responsive.
2. **Glassmorphism Loading HUD**: During checkout, we show a sleek, lightweight loading overlay to prevent accidental double-taps.
3. **Immediate Wallet Sync**: The moment the secure Supabase Edge Function validates the purchase, the local Riverpod wallet state updates instantly. There are no screen reload delays.
4. **Resilient Receipt Recovery (No Lost Purchases)**: 
   * If a user loses internet connection immediately after paying but before the Supabase database can verify it, the app **saves the transaction receipt locally** in secure storage.
   * On the next app launch or as soon as internet is restored, the app automatically retries verification in the background, crediting the user's coins or Premium status without requiring them to purchase again or contact support.
